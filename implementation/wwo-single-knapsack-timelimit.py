            ...
            if strategy == 0:
                ...
                if selected and unselected:
                    for _ in range(min(k, len(selected), len(unselected))):
                        ...
                        for sel_item in selected[:min(3, len(selected))]:  # Limit search for speed
                            for unsel_item in unselected[:min(3, len(unselected))]:
                                temp_solution = neighbor.solution[:]
...
class WWOSingleKnapsackTimeLimited:
    """Time-limited Water Wave Optimization Algorithm for Single Knapsack Problem"""
    ...
    def run(self) -> KnapsackSolution:
        """Run the time-limited WWO algorithm"""
        ...
        if not self.population:
            # Fallback: create a simple solution
            fallback = KnapsackSolution(self.instance)
            return fallback
            ...
            for solution in self.population:
                ...
                if new_solution.fitness > solution.fitness:
                    ...
                    if new_solution.fitness > self.best_solution.fitness:
                        ...
                        if self.time_remaining() > 0.1:  # Leave time buffer
...
def solve_single_knapsack_time_limited():
    """Main function to solve single knapsack problem with time limit"""
    try:
        # Load instance from stdin
        instance = SingleKnapsackInstance()
        
        # Adjust algorithm parameters based on problem size for time efficiency
        ...
        # Run time-limited WWO algorithm
        wwo = WWOSingleKnapsackTimeLimited(
            instance, 
            np_max=np_max,
            np_min=max(6, np_max // 4),
            n_b=n_b,
            time_limit=4.5  # 4.5 seconds
        )
...
if __name__ == "__main__":
    solve_single_knapsack_time_limited()

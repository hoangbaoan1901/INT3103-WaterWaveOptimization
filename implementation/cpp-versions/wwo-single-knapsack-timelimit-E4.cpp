...
public:
    ...
    void calculate_wavelengths() {
        ...
        // Calculate wavelength using E4: λ_x = λ_min + (λ_max - λ_min) * (f_max - f(x) + ε) / (f_max - f_min + ε)
        for (auto& solution : population) {
            if (abs(f_max - f_min) < epsilon) {
                ...
                solution.wavelength = lambda_min + (lambda_max - lambda_min) * 
                                    (f_max - solution.fitness + epsilon) / (f_max - f_min + epsilon);
            }

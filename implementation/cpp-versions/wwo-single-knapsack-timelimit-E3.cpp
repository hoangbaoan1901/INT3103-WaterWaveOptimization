...
public:
    ...
    KnapsackSolution run() {
        ...
        while (time_remaining() > 0) {
            ...
            // If stagnated for too long, apply diversification
            if (stagnation_counter >= 10 && time_remaining() > 1.0) {
                // Intensified local search around best solution
                vector<KnapsackSolution> intensive_neighbors = best_solution->local_search_breaking(n_b * 2, rng);
                ...
                // If still no improvement, restart part of population
                if (stagnation_counter >= 10) {
                    int restart_count = population.size() / 2;
                    for (int i = 0; i < restart_count && i < (int)population.size(); i++) {

...
public:
    ...
    vector<int> generate_random_solution() {
        ...
        // Add some randomness by shuffling top third
        if (ratios.size() > 6) {
            int top_third = ratios.size() / 3;
            shuffle(ratios.begin(), ratios.begin() + top_third, default_random_engine());
        }
...
public:
    ...
    void calculate_wavelengths() {
        ...
        // Calculate sum of all positive fitness values
        ...
        for (auto& solution : population) {
            if (total_fitness > 0) {
                ...
                double ratio = (total_fitness - normalized_fitness) / total_fitness;
                solution.wavelength = lambda_max * ratio;
...
int main() {
    ...
    try {
        ...
        WWOSingleKnapsack wwo(&instance, np_max, max(6, np_max / 4), lambda_max, 1, 3, 5.0);

...
public:
    ...
    KnapsackSolution copy() {
        return KnapsackSolution(instance, &solution);
    }
...
public:
    ...
    void calculate_wavelengths() {
        ...
        // Find f_max and f_min in population
        double f_max = -1e9, f_min = 1e9;
        for (const auto& sol : population) {
            f_max = max(f_max, sol.fitness);
            f_min = min(f_min, sol.fitness);
        }
        
        double epsilon = 1e-6;
        double b = (double)lambda_max / lambda_min;  // Control parameter
        double alpha = 1.0;  // Set to 1 for convenience as mentioned in the paper
        
        // Calculate wavelength using E5: λ_x = λ_min * b^(α * (f_max - f(x) + ε) / (f_max - f_min + ε))
        for (auto& solution : population) {
            if (abs(f_max - f_min) < epsilon) {
                // If all solutions have same fitness, set to middle value
                solution.wavelength = (lambda_max + lambda_min) / 2.0;
            } else {
                double exponent = alpha * (f_max - solution.fitness + epsilon) / (f_max - f_min + epsilon);
                solution.wavelength = lambda_min * pow(b, exponent);
            }
